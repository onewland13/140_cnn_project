...
def main(a):
	...
	df.to_csv('./results/mnist_SRAM_fault_sign_bit_preserving_RESULTS.csv')
...
class ConvNet(nn.Module):
			...
			for i_vector in range(len(self.fc1.weight)):
				for i_w in range(len(self.fc1.weight[i_vector])):
					if random.random() < self.fault_rate:
						...
						self.fc1.weight[i_vector][i_w] = self._sign_random_bit(weight)
			# evaluate weights
			out = self.fc1(out)
			# change weights back 
			...
			for i_vector in range(len(self.fc2.weight)):
				for i_w in range(len(self.fc2.weight[i_vector])):
					if random.random() < self.fault_rate:
						...
						weights_changed.append((i_vector, i_w, weight))
						self.fc2.weight[i_vector][i_w] = self._sign_random_bit(weight)
			# evaluate weights
			out = self.fc2(out)
			# change weights back 
			for i_v, i_w, w in weights_changed:
	...
	# here our fault rate handling procedure is to set the bit to the sign bit
	def _sign_random_bit(self, w):
		rep = struct.pack('>f', w)
		...
		# sign bit is 1
		if w < 0:
			rep = struct.pack('>l', integer_w | (1 << bit_loc))
		# sign bit is 0
		else: 
			rep = struct.pack('>l', integer_w & ~(1 << bit_loc))
